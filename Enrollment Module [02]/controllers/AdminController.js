...
/////////////////////// REGISTRATION MODULE ///////////////////////
export const getCourseRegistrationtatus = async(req, res) => {
    try{
        const { sem, batch } = req.body
        const enrollmentstatus = await SemesterMetadataModel.findOne({semester:{sem:sem,batch:batch}})
        // console.log(enrollmentstatus);
...
export const modifyCourseRegistrationStatus = async(req, res) => {
    try{
        const { sem, batch } = req.body
...
export const getRegisteredstudentslist = async(req, res) => {
    try{
        ...
        const data = await EnrollmentModel.find({batch:batch, branch:branch,semester:sem,enrolled:true}, {courseCode:1,studentId:1,branch:1,enrolled:1,approval:1,_id:0}).populate("courseCode", {courseCode:1,title:1}).populate("studentId",{firstName:1,register:1,branch:1,batch:1})     
    ...
    try{
        ...
        for(let course of courses){          
            ...
            for(let student of course.students){
                ...
                if(student.approval == 6){
                    enrollmentdata.approval = 6
                }
...
export const cr_addstudents = async(req, res) => {
    try{
        const {courses} = req.body
...
export const cr_removestudents = async(req, res) => {
    try{
        ...
        for(let course of courses){          
            ...
            for(let student of course.students){
                ...
                await EnrollmentModel.findOneAndDelete({courseCode:courseinfo._id,studentId:studentinfo._id})
                // console.log(foundenrollmentdata)
