...
export const CE_Student_getenrolmentdata = async(req, res) => {
    try{
        let allowedpeelectives=[], allowedoeelectives=[], addonallowed=false, addontype, oecourses, pecourses,electivesallowed=false,mandatorycourses = [],previouslyenrolledelectives=[], ispreviouslyenrolledelectives=false
        // const studentmail = "abir.1918101@gct.ac.in"
        const studentmail = "abiramee1128@gmail.com"
        const studentinfo = await StudentsModel.findOne({email:studentmail})
        if(!studentinfo){
        ...
        if(enrollmentstatus){
            ...
            for(let each of mandatorycoursesindb){
                const foundinenrollment = await EnrollmentModel.findOne({studentId:studentinfo._id, courseCode:each._id,enrolled:false})
        ...
        if(allowedelectivecourses){
            ...
            // allowedoeelectives = allowedelectivecourses.oe
            // allowedpeelectives = allowedelectivecourses.pe

            pecourses = await CurriculumModel.find({branch:studentinfo.branch,category:"PE"})
            // console.log(pecourses)
            oecourses = await CurriculumModel.find({category:"OE"})
            ...
            if(semesterdataforaddon){
                // allowedpeelectives
                addonallowed = true
                addontype = semesterdataforaddon.addOnEligible[0].course
            }
            
            const foundinenrollment = await EnrollmentModel.findOne({studentId:studentinfo._id, courseCategory:addontype}).populate("courseCode")
            if(foundinenrollment && foundinenrollment.semester!=studentinfo.currentSemester){
                // which means already he has studied this addon
                addonallowed = false
                addontype = undefined
            }
            if(foundinenrollment&& foundinenrollment.semester == studentinfo.currentSemester){
                const obj = {
                    courseType:foundinenrollment.courseCategory,
                    courseCode:foundinenrollment.courseCode.courseCode,
                    title:foundinenrollment.courseCode.title,
                    previouslyenrolledforthiselective:true
                }
